//本js直接将必备的iframeTools和artDialog扩展与artDialog整合在一起，便于引入
//引入时只需要引入(skins为使用的皮肤)
//<script src="artDialog/jquery.artDialog.js?skin=twitter"></script>(注意。该skin末尾不带s，拥有的样式详见skins目录)
//或
//<script src="artDialog/jquery.artDialog.js"></script>
//<link rel="stylesheet" href="artDialog/skins/twitter.css">
...
//自带（虽是自带，但并未加入artDialog中）的扩展shake、notice、 tips
;(function(artDialog){
	// 左右晃动效果
...
//设定缺省参数
(function (config) {
    config['lock'] = true;   //使用遮罩
    config['fixed'] = false; //false，窗口随滚动条滚动（true则固定在可视范围）
    config['esc'] = false;   //防止loading能被esc给取消
})(art.dialog.defaults);

//扩展 $.dialog.mask(name),,,$.dialog.unmask(name)
;(function($, artDialog){
	...
	$.extend(artDialog, {
		/**
		 * 显示loading
		 * @param name loading标识
		 */
		mask : function(name){
		    var loading = $.dialog({title: false, cancel:false});
		    loadingMgr[name] = loading;
		},
		...
		unmask : function(name){
		    var loading = loadingMgr[name];
		    if(loading){
		    	loading.close();
		    	delete loadingMgr[name];
		    }
		}
	});
})(jQuery, artDialog);