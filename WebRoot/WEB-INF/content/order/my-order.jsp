...
<section>
	...
	<c:if test="${not empty  list2 && listSize > 0}">
	<c:forEach items="${list2}" var="childrenRow" varStatus="i"  >
		<c:choose>
				<c:when test="${i.last}">
					   <div class="side_list">
	...
	<c:if test="${ listSize ==0}">
		<span style="display:table; padding:30px 10px; line-height:30px;">亲爱的客官，您还没有点餐或已点餐但未下单哟～</span>
	</c:if>
    </div>
    <div class="cp_list2">
    	<c:if test="${listSize >0 }">
